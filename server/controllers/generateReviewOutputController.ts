        ...
        if(valvPmsItems.length > 0){
            for (const valvItem of valvPmsItems) {
                ...
                valvProcessedItems.push({
                    ...
                    CompType: component.componentname,
                    ShortCode: componentDesc.short_code,
                    ItemCode: `${componentDesc.code}${sizeData?.code}${'X'}${schedule1 ? scheduleCode1?.schedule_code : 'XX'}${schedule2 ? scheduleCode2?.schedule_code : 'XX'}${rating ? rating.ratingCode : 'X'}${material?.code}`,
                    CItemCode: `${componentDesc.c_code}${sizeData?.c_code}${'X'}${schedule1 ? schedule1.c_code : 'XX'}${schedule2 ? schedule2.c_code : 'XX'}${rating ? rating.c_rating_code : 'X'}${material?.c_code}`,
                    ...
                    Sch1: schedule1?.sch1_sch2 ?? 'XX',
                    Sch2: schedule2?.sch1_sch2 ?? 'XX',
