...
const PricingCard = ({ name, Price, benefits, index, showButton, openModal }: PricingCardProps) => {
  return (
    <div className="border py-10 md:px-6 px-4 rounded-lg shadow-3xl flex flex-col justify-between h-full">
      {/* Card Content */}
      <div>
      ...
      {showButton && (
        <div className="w-full mx-auto flex items-center justify-center mt-5">
          <button
            ...
            onClick={() => openModal?.(index)}  // Call openModal only if it's provided
          >
            Get Started
          </button>
        </div>
